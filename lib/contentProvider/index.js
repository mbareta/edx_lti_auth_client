...
const getDeliverableContentTreeWithData = (email, key) => {
  return responsesRepository
  .getDeliverableByType(email, key)
  .then(deliverableData => {
    const deliverable = getDeliverable(key);

    deliverableData.forEach(item => {
      const subDeliverableKey = item.subType;
      const activityKey = item.name;

      deliverable.subDeliverables[subDeliverableKey].activities[activityKey].data = item.data;
    })
