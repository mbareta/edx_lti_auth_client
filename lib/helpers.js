...
const isLtiSession = (req: express$Request) => {
  const ltiEmail = req.session.lti && req.session.lti.email;
  let ltiRequestEmail;
  if(req.body && req.body.lti_version) {
    if (req.body.user_id === 'student') {
  ...
  return ltiEmail || ltiRequestEmail || false;
}

module.exports = { excludesRequestRoute, skipWhitelistedRoutes, intercept, getEmailFromSession, isLtiSession }
