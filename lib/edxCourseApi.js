...
class EdxCourseApi {
  ...
  getCourseLtiBlocks(req, { blocks_url }, options) {
    const username = req.session.user.preferred_username;

    // TODO: test if we can remove the if-else since username should always be present for logged in user
    if (username) {
      options.url = `${blocks_url}&username=${username}&depth=all&block_types_filter=lti,lti_consumer&return_type=list&requested_fields=launch_url`;
    }
    else {
  ...
  getAllUserBlocks(req) {
    ...
    .map(course => this.getCourseLtiBlocks(req, course, options))
